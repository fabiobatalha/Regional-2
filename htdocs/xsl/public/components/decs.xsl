...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="decs">
        <div id="decs">
            <script language="JavaScript" src="&lt;?=$def['DIRECTORY']?&gt;js/yui/yahoo.js"><!-- yui library -->
            </script>
            <script language="JavaScript" src="&lt;?=$def['DIRECTORY']?&gt;js/yui/dom.js"><!-- yui library -->
            </script>
            <script language="JavaScript" src="&lt;?=$def['DIRECTORY']?&gt;js/yui/event.js"><!-- yui library -->
            </script>
            <script language="JavaScript" src="&lt;?=$def['DIRECTORY']?&gt;js/yui/connection.js"><!-- yui library -->
            </script>
            <script language="JavaScript" src="&lt;?=$def['DIRECTORY']?&gt;js/yui/autocomplete.js"><!-- yui library -->
            </script>
                ...
                <span>
                    <xsl:choose>
                        <xsl:when test="$decsData/@href != ''">
                            <a href="{$decsData/@href}">
                                <xsl:value-of select="$decsData/text()" />
                            </a>
                        </xsl:when>
                        <xsl:otherwise>
                            <a href="../php/level.php?lang={$lang}&amp;component={$id}">
                                <xsl:value-of select="$decsData/text()" />
                            </a>
                        </xsl:otherwise>
                    </xsl:choose>
            ...
            <div class="label">
                <xsl:value-of select="$decsTexts/text[@id = 'search_terms']"/>
            </div>
            ...
            <div class="label">
                <xsl:value-of select="$decsTexts/text[@id = 'category']"/>
            </div>
            <ul>
