...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="metasearch">
        ...
        <div id="search" style="display: block;">
            ...
            <form name="searchForm" action="#" method="POST" onsubmit="return(executeSearch());">
                ...
                <xsl:if test="//url_search">
                    <input type="hidden" name="url_search" value="{//url_search[1]}"/>
                </xsl:if>
                ...
                <div class="searchItens">
                    <div id="method">
                        ...
                        <select name="engine">
                            <option value="applications/scielo-org/iahx"><xsl:value-of select="$metaTexts//labels[@lang=$lang]/metaSearch/iahx" /></option>
                    ...
                    <input type="submit" value="{text[@id = 'search_submit']}" class="submit" />
                    <br/><br/>
    ...
    <xsl:template match="*" mode="getLinks">
        <xsl:if test="position() != 1">,&#160;</xsl:if><a href="javascript: document.searchForm.url.value='{.}'; document.searchForm.mode.value='index'; if (executeSearch()) document.searchForm.submit();"><xsl:value-of select="@name" /></a>
    </xsl:template>
