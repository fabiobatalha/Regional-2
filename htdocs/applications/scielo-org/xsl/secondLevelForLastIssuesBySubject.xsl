...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:variable name="subjectsChoosed">
        <xsl:choose>
            <xsl:when test="$subject != ''">
                <subjectList>
                    <xsl:copy-of select="document(concat($PATH,'/xml/',$lang,'/subjectList.xml'))/subjectList/subject[name = $subject]"/>
                </subjectList>
            </xsl:when>
            <xsl:otherwise>
                <xsl:copy-of select="document(concat($PATH,'/xml/',$lang,'/subjectList.xml'))"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:variable>
    <xsl:variable name="subjects" select="document(concat($PATH,'/xml/',$lang,'/subjectList.xml'))"/>
    <xsl:template match="/">
        <div class="middle">
            <div id="breadCrumb">
            ...
            <div class="serviceColumn">
                <div class="webServices">
                    ...
                    <div id="sortedBy">
                        <ul>
                            <li>
                                <a href="{$DIR}/applications/scielo-org/php/secondLevel.php?xml=secondLevelForLastIssuesByAlphabeticList&amp;xsl=secondLevelForLastIssuesByAlphabeticList{$instanceParam}" class="closed">
                            ...
                            <xsl:if test="$scielo_portal='yes'">
                                <xsl:if test="normalize-space($instance) = ''">
                                    <li>
                                        <a href="{$DIR}/applications/scielo-org/php/secondLevel.php?xml=secondLevelForLastIssuesByAlphabeticList&amp;xsl=secondLevelForLastIssuesByCollection{$instanceParam}" class="closed">
                            ...
                            <li>
                                <a href="{$DIR}/applications/scielo-org/php/secondLevel.php?xml=secondLevelForLastIssuesBySubject&amp;xsl=secondLevelForLastIssuesBySubject{$instanceParam}" class="closed">
            ...
            <div class="content">
                <h3>
                    <span>
                        <xsl:value-of select="$texts/text[find='lastIssuesBySubject']/replace"/>
    ...
    <xsl:template match="journal">
        <li>
            ...
            <xsl:if test="$scielo_portal='yes'">
<span class="collectionName"> [<xsl:value-of select="../../@collection"/>]</span></xsl:if>
    ...
    <xsl:template match="letter">
        ...
        <xsl:if test="count($count[@by=$letter]//journal) &gt; 0">
            <a href="{$DIR}/applications/scielo-org/php/secondLevel.php?xml=secondLevelForLastIssuesByAlphabeticList&amp;xsl=secondLevelForLastIssuesByAlphabeticList&amp;letter={.}" title="">
                <xsl:value-of select="."/>
            </a>
        </xsl:if>
    </xsl:template>
    <xsl:template match="*" mode="getSubjectList">
        ...
        <xsl:if test="$count &gt; 0">
            <li>
                <a href="{$DIR}/applications/scielo-org/php/secondLevel.php?xml=secondLevelForLastIssuesBySubject&amp;xsl=secondLevelForLastIssuesBySubject&amp;instance={$instanceParam}&amp;subject={name}" title="">
                    <xsl:value-of select="translate"/>
                </a>
                <br/>
            </li>
